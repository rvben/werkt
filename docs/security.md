...
Builds receive `runtime.environment` but never `runtime.secrets`. Both process builds and local runtime attempts start with a small allowlist of ordinary host variables such as `PATH`, `HOME`, locale, temporary-directory, and certificate paths; Werkt database, management, Husker, and vault credentials are not inherited. Runtime vault values go only to the attempt process or fresh Husker VM. A `runtime.tools` preparation VM receives neither source nor secrets. Its network is filtered to the built-in catalog entry, and authors cannot override its `PATH`, `HOME`, or `MISE_` settings.
